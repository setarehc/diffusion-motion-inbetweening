...
def evaluate_matching_score(eval_wrapper, motion_loaders, file):
    ...
    keyframe_score_dict = OrderedDict({})
    ...
    for motion_loader_name, motion_loader in motion_loaders.items():
        ...
        with torch.no_grad():
            for idx, batch in enumerate(motion_loader):
                ...
                if motion_loader_name == "vald":
                    ...
                    keyframe_err_np = calculate_keyframe_error(keyframe_error, num_keyframes)
                    keyframe_err.append(keyframe_err_np)
        ...
        if motion_loader_name == "vald":
            ...
            keyframe_err = np.stack(keyframe_err).mean(0)
            keyframe_score_dict[motion_loader_name] = keyframe_err
            print(f'---> [{motion_loader_name}] Keyframe Error: {keyframe_err:.4f}')
            print(f'---> [{motion_loader_name}] Keyframe Error: {keyframe_err:.4f}', file=file, flush=True)
    ...
    return match_score_dict, R_precision_dict, activation_dict, trajectory_score_dict, skating_ratio_dict, keyframe_score_dict
    ...
    with open(log_file, 'w') as f:
        ...
        for replication in range(replication_times):
            ...
            (mat_score_dict, R_precision_dict, acti_dict,
             trajectory_score_dict, skating_ratio_dict, keyframe_score_dict) = evaluate_matching_score(
            ...
            for key, item in keyframe_score_dict.items():
                if key not in all_metrics['Keyframe Error']:
                    all_metrics['Keyframe Error'][key] = [item]
                else:
                    all_metrics['Keyframe Error'][key] += [item]
...
def load_traj_model(data):
    ...
    args_traj = our_eval_args(model_path=traj_model_path)
...
if __name__ == '__main__':
    ...
    split = 'test'
    gt_loader = load_dataset(args, args.num_frames, split, hml_mode='gt') # NOTE: gt_loader.dataset.absolute_3d=True but get_opt.py ignores use_abs3d if mode in ['eval', 'gt']
    gen_loader = load_dataset(args, args.num_frames, split, hml_mode='eval') # NOTE: gen_loader.dataset.absolute_3d=True but get_opt.py ignores use_abs3d if mode in ['eval', 'gt']
    ...
    eval_motion_loaders = {
        ...
        ################
        'vald':
        lambda seed: get_mdm_loader_ours(model_dict=model_dict, diffusion_dict=diffusion_dict, batch_size=args.batch_size, ground_truth_loader=gen_loader,
                                         mm_num_samples=mm_num_samples, mm_num_repeats=mm_num_repeats, max_motion_length=gt_loader.dataset.opt.max_motion_length,
                                         num_samples_limit=num_samples_limit, text_scale=args.guidance_param, keyframe_scale=args.keyframe_guidance_param, seed=seed,
                                         save_dir=save_dir, impute_until=impute_until, skip_first_stage=skip_first_stage, use_ddim=args.use_ddim, args=args),
    }
